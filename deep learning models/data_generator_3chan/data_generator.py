...
class DataGenerator:
    ...
    def load_hdf5_dataset(self, verbose=True):
        ...
        if self.hdf5_dataset.attrs['has_label']:   # due to the typo in create_hdf5_datasetZZ
            self.labels = []
...
#                    image = image[:,:,:3]
            ...
            # make sure all images have 3 channels (end with 256 x 256 x 3)
                    ...
                    for transform in transformations:
                        ...
                        if not (self.labels is None):
                            ...
                            if ('inverse_transform' in returns) and ('return_inverter' in inspect.signature(transform).parameters):
                                ...
                                ## modification: make sure image has shape 256 x 256 x 3 after transform
                               # if batch_X[i].ndim == 2:
