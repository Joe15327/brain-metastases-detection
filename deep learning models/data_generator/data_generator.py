...
class DataGenerator:
    ...
    def load_hdf5_dataset(self, verbose=True):
        ...
        if self.hdf5_dataset.attrs['has_label']:  ## because of the typo below in creat_hdf5_datasetZZ, 'attrs.modify(name = 'has_label')'
            self.labels = []
...
###############################################
#     custome the create_hdf5_dataset method
###############################################        
    def create_hdf5_datasetZZ(self,
                              imgSet,
                              file_path='dataset.h5',
                              resize=False,
                              variable_image_size=True,
        ...
        self.hdf5_dataset_path = file_path
        dataset_size = imgSet.shape[2] # slice number is 1st or 3rd diminsion
        ...
        # create the dataset and store image as flattened arrays
        hdf5_images = hdf5_dataset.create_dataset(name = 'images',
                                                  shape = (dataset_size,),
                                                  maxshape = (None),
                                                  dtype = h5py.special_dtype(vlen = np.float64))
        
        # create the dataset that holds the image heights, width and channels
        hdf5_image_shapes = hdf5_dataset.create_dataset(name = 'image_shapes',
                                                        shape = (dataset_size, 3),
                                                        maxshape = (None, 3),
                                                        dtype = np.int32)
        if not (self.labels is None):
            # create the dataset and store labels as flattened arrays
            hdf5_labels = hdf5_dataset.create_dataset(name = 'labels',
                                                      shape = (dataset_size,),
                                                      maxshape = (None),
                                                      dtype = h5py.special_dtype(vlen = np.int32))
            
            # create the dataset that hold the dimension of labels
            hdf5_label_shapes = hdf5_dataset.create_dataset(name = 'label_shapes',
                                                            shape = (dataset_size, 2),
                                                            maxshape = (None, 2),
                                                            dtype = np.int32)
            
            hdf5_dataset.attrs.modify(name = 'has_label', value = True)
            
        # similar thing for image_ids and eval_neutral
        if not (self.image_ids is None):
            hdf5_image_ids = hdf5_dataset.create_dataset(name = 'image_ids',
                                                         shape = (dataset_size,),
                                                         maxshape = (None),
        ...
        for i in tr:
            ...
            # store the evaluation-neutral annotations if any
            if not(self.eval_neutral is None):
                hdf5_eval_neutral[i] = self.eval_neutral[i]
            
            # store the image, make sure the slice index is the same as image_ids
            if not(self.image_ids is None):
                index = int(self.image_ids[i])-1 # image_ids start from 1
                image = imgSet[:,:,index]
            else:
                image = imgSet[:,:,i]
...
#                    image = image[:,:,:3]
            ...
            # make sure all images have 1 channel (end with 256 x 256 x 1)
            image = np.reshape(image, image.shape + (1,))

            # flatten the image array and write it to the images dataset
            hdf5_images[i] = image.reshape(-1)
            # write the image's shape to the image_shape dataset
            hdf5_image_shapes[i] = image.shape
                    ...
                    for transform in transformations:
                        ...
                        if not (self.labels is None):
                            ...
                            if ('inverse_transform' in returns) and ('return_inverter' in inspect.signature(transform).parameters):
                                ...
                                ## modification: make sure image has shape 256 x 256 x 1 after transform
                                if batch_X[i].ndim == 2:
                                    batch_X[i] = np.reshape(batch_X[i], batch_X[i].shape + (1,))
                                elif batch_X[i].ndim > 2:
